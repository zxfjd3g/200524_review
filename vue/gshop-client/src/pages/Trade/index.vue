  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="address in tradeInfo.userAddressList" :key="address.id">
        <!-- selected  -->
        <span class="username" :class="{selected: selectedAddress===address}" @click="selectedAddress=address">{{address.consignee}}</span>
...
<script>
  ...
  export default {
    ...
    watch: {
      // 监视它的变化, 请求得到tradeInfo更新后调用
      // watch也可以只监视data属性内部的某个属性数据
      'tradeInfo.userAddressList': function (value) {   // tradeInfo = {}   tradeInfo = {...}
        // 得到默认的address
        const address = value.find(item => item.isDefault==='1')
        // 更新为选中的address
        if (address) {
